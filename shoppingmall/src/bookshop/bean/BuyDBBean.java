...
public class BuyDBBean {
	...
	public void insertBuy( List<CartDataBean> lists,
        ...
        try {
            ...
            // 위의 sql문으로 가져오는 값은 어차피 1개 (buy_id의 최댓값)이므로 rs.next()를 루프 안에서 돌리는 게 아니라 그냥 가져오는 것
            rs.next();
            if (rs.getLong(1) > 0){         
...
//                int num = pstmt.executeUpdate();				// 조회 빼고 나머지는 다 executeUpdate() 사용. 처리된 행의 수를 int형으로 반환함.
//                if(num == 0) System.out.println("처리 실패");		// 디버깅 단계. 위에 있는 num 변수는 여기서 사용함.
//                else System.out.println("처리 성공");
...
//              int num = pstmt.executeUpdate();				// 디버깅하는 코드. 위의 한 줄 삭제하고 돌리면 됨
//              if(num == 0) System.out.println("처리 실패");		
//              else System.out.println("처리 성공");
            }
